...
static class Elegant
{
  ...
  public static IEnumerable<ArraySegment<long>> GroupByBlocks(long[] disk) => disk.GroupByBlocksOfContiguousEqualElements().Select(x => new ArraySegment<long>(disk, x.start, x.length));
